...
namespace RealEstates.ConsoleApplication
{
    class Program
    {
        ...
        private static void GetPropertiesrByTagName(ApplicationDbContext db)
        {
            IPropertiesService propertiesService = new PropertiesService(db);
            Console.WriteLine("Enter tags names:");
            string[] tags = Console.ReadLine().Split((", "));
            var result = propertiesService.SearchByTags(tags);

            foreach (var p in result)
            {
                Console.WriteLine($"{p.DistrictName}, {p.Type} {p.Floor} - {p.Size} - {(p.Price*1.13):F2}$");
            }
        }
