    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (StreamWriter writer = new StreamWriter
                (@$"{Environment.GetFolderPath(Environment.SpecialFolder.Desktop)}\FilesInformation.txt"))
            {
                foreach (var item in fileInfo.OrderByDescending(x => x.Value.Count).ThenBy(n => n.Key))
                {
